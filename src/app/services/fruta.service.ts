...
export class FrutaService {
  ...
  url = 'https://localhost:44335/api/Produtos'; // api swagger
  ...
  updateFruta(fruta: Fruta): Observable<Fruta> {
    return this.httpClient.put<Fruta>(this.url + '/' + fruta.id, JSON.stringify(fruta), this.httpOptions)
      .pipe(
        retry(1),
        catchError(this.handleError)
      )
  ...
  deleteFruta(fruta: Fruta) {
    return this.httpClient.delete<Fruta>(this.url + '/' + fruta.id, this.httpOptions)
      .pipe(
        retry(1),
        catchError(this.handleError)
      )
